...
public class ShiroAutoConfiguration {
    ...
    public ShiroFilterFactoryBean getShiroFilterFactoryBean(DefaultWebSecurityManager defaultWebSecurityManager){
        ...
        //配置拦截的路径
        String[] authUrls = this.shiroProperties.getAuthcUrls();
        if (authUrls != null && authUrls.length > 0){
            for (String authUrl : authUrls){
                map.put(authUrl,"authc");
            }
        }
    ...
    public FilterRegistrationBean<DelegatingFilterProxy> registDelegatingFilterProxy(){
        ...
        delegatingFilterProxy.setTargetBeanName(SHIRO_FILTER_NAME);
        ...
        filterRegistrationBean.setServletNames(servleNames);
